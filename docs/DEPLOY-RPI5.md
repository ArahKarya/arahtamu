...
Pola deploy default ArahKarya untuk on-prem / RPi5: **Docker Compose** + **Cloudflare Tunnel**, BUKAN nginx port-forward. Tidak buka port di router, tidak butuh static IP.

> Tunnel name yang dipakai untuk semua app ArahKarya: **`arahkarya`** (satu tunnel, banyak hostname routing).
...
- Akun Cloudflare + domain (contoh aktif: `arahkarya.com`, `panggonmikir.arahkarya.com`, `kuttabkemiling.arahkarya.com`)
...
#   CORS_ORIGIN=https://<app>.arahkarya.com
#   APP_URL=https://<app>.arahkarya.com
...
cloudflared tunnel create arahkarya   # buat tunnel "arahkarya"
# output: Created tunnel arahkarya with id <UUID>
...
ingress:
  ...
  - hostname: kuttabkemiling.arahkarya.com
    service: http://localhost:3008
  # NEW APP — sesuaikan port:
  - hostname: <app>.arahkarya.com
...
```bash
cloudflared tunnel route dns arahkarya <app>.arahkarya.com
```
...
Akses: `https://<app>.arahkarya.com` → harus muat login page.

## Step 4 — Backup harian (cron)

```bash
# /etc/cron.d/arahkarya-backup
0 2 * * * <user> cd /home/<user>/<app> && ./scripts/backup-db.sh >> /var/log/arahkarya-backup.log 2>&1
```
...
| `502 Bad Gateway` di `<app>.arahkarya.com` | App container tidak listen di port yang ada di `config.yml`. Cek `docker ps`, samakan `service:` di config. |
| Tunnel down | `sudo systemctl restart cloudflared`. Status: `cloudflared tunnel info arahkarya`. |
...
- **Health check**: `curl https://<app>.arahkarya.com/api/health` — harus return `{ status: 'ok', db: 'up', redis: 'up' }`.
